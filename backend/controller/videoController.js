...
import uploadOnCloudinary, { cloudinary } from "../config/cloudinary.js";
import User from "../model/userModel.js";


export const createVideo = async (req, res) => {
  try {
    const { title, description, tags, channel, videoUrl, videoPublicId, thumbnailUrl, thumbnailPublicId } = req.body;

    if (!title || !channel || !videoUrl || !videoPublicId || !thumbnailUrl || !thumbnailPublicId) {
      return res.status(400).json({
        message: "title, channel, videoUrl, videoPublicId, thumbnailUrl, and thumbnailPublicId are required"
      });
    }
  ...
  try {
    ...
    if (video.publicId) {
      try {
        ...
        console.error("Failed to delete video asset from Cloudinary", destroyError);
      }
    }
    ...
    if (video.thumbnailPublicId) {
      try {
        ...
        console.error("Failed to delete thumbnail asset from Cloudinary", destroyError);
      }
    }
