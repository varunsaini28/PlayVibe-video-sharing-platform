...
export const createShort = async (req, res) => {
  try {
    const { title, description, tags, channelId, shortUrl, publicId } = req.body;

    if (!title || !shortUrl || !publicId || !channelId) {
      return res.status(400).json({ message: "Title, shortUrl, publicId, and channelId are required" });
    }
...
export const deleteShort = async (req, res) => {
  try {
    ...
    if (short.publicId) {
      try {
        await cloudinary.uploader.destroy(short.publicId, { resource_type: "video" });
      } catch (destroyError) {
        console.error("Failed to delete short asset from Cloudinary", destroyError);
      }
    }
